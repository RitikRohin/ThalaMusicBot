...
@app.on_callback_query(filters.regex("GroupStream") & ~BANNED_USERS)
@languageCB
async def play_Music(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    vidid, user_id, mode, gplay, fplay = callback_request.split("|")
    ...
    try:
        chat_id, title = await get_groupplayCB(_, gplay, CallbackQuery)
        if not chat_id:
            return
    except:
        return
    ...
    mystic = await CallbackQuery.message.reply_text(
        _["play_2"].format(title) if title else _["play_1"]
    )
